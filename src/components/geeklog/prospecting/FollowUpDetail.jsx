    ...
    <div style={{ padding: "0 20px 40px" }}>
      ...
      {log && (
        <div style={{ marginTop: 22, border: `1px solid ${T.line}`, borderRadius: 12, background: T.surface, padding: "13px 15px" }}>
          ...
          <div style={{ fontSize: 14, color: T.cream, fontFamily: FF.sans }}>
            {formatTouchDate(log.ts)}
            {log.outcome ? ` · ${log.outcome}` : ""}
            {log.score ? ` · ${log.score}/10` : ""}
          </div>
          {log.note && <div style={{ fontSize: 13.5, color: T.dim, marginTop: 6, lineHeight: 1.5, fontFamily: FF.sans, whiteSpace: "pre-wrap" }}>{log.note}</div>}
        </div>
      )}

      <div style={{ marginTop: 26 }}>
        <h2 style={{ fontFamily: FF.serif, fontWeight: 400, fontSize: 22, marginBottom: 12, color: T.cream }}>Log a follow up</h2>
        <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="What happened on this touch..."
          style={{ width: "100%", minHeight: 88, resize: "vertical", background: T.surface, color: T.cream, border: `1px solid ${T.line}`, borderRadius: 10, padding: 12, fontFamily: FF.sans, fontSize: 15, lineHeight: 1.5 }} />
        <button type="button" onClick={submit} disabled={!note.trim()}
          style={{ width: "100%", marginTop: 12, padding: 16, borderRadius: 12, border: "none", background: note.trim() ? T.green : T.surface, color: note.trim() ? T.cream : T.faint, fontFamily: FF.sans, fontSize: 16, fontWeight: 700, cursor: note.trim() ? "pointer" : "default" }}>
          Log follow up
        </button>
      </div>
          ...
          {history.map((t, i) => (
            <details key={`${t.ts}-${i}`} style={{ border: `1px solid ${T.line}`, borderRadius: 10, overflow: "hidden", marginBottom: 8 }}>
              <summary style={{ listStyle: "none", display: "flex", alignItems: "baseline", gap: 10, padding: "12px 14px", cursor: "pointer", background: T.surface }}>
                ...
                <span style={{ flex: 1, minWidth: 0, fontSize: 13, color: T.dim, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontFamily: FF.sans }}>{t.note}</span>
              </summary>
              <div style={{ padding: "12px 14px", fontSize: 14, lineHeight: 1.55, color: T.cream, background: T.bg0, whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: FF.sans }}>{t.note || "No note"}</div>
            </details>
          ))}
        </div>
      )}
